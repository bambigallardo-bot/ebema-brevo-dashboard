...
export async function getWhatsappClicksByCampaign() {
  ...
  // Métrica de "clic": por defecto sesiones. Se puede cambiar con GA_CLICK_METRIC.
  const metric = process.env.GA_CLICK_METRIC || "sessions";
  // Filtro de canal opcional: solo se aplica si defines GA_WHATSAPP_VALUE.
  // Por defecto NO se filtra por source/medium; el match es por utm_campaign.
  const waValue = process.env.GA_WHATSAPP_VALUE;
